...
@pytest.mark.parametrize("chain", all_chains)
def test_get_transaction_count(evm_client: EVMClient, chain: str, binance_hot_wallet) -> None:
    result = evm_client._client.eth.get_transaction_count(evm_client._chain_config.wallet_address)
    print(result)
